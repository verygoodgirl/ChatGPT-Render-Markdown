ChatGPT — Render Markdown in YOUR messages 
Script version: 0.5.0 · Updated: 2025-08-19 

What this is: a Tampermonkey userscript that intercepts your own messages in ChatGPT’s web UI and locally renders italic, bold, bold+italic, strikethrough, code, links, headings and lists — on your side only. 


Features 
• Renders *italic*, **bold**, ***bold+italic***, ~~strikethrough~~. 
• Supports [links](https://example.com), headings #/##/###, unordered lists (- or *) and task checkboxes [ ] / [x]. 
• Hotkey Alt+M — instant toggle; state persists. Small “MD ON/OFF” badge in the bottom-left corner. 
• Opt-out: if a message contains the sequence {{{ it will be left raw (useful for examples). 
• Safe: the server and the assistant still receive your original text — only the local rendering is changed. 


Install (Chrome/Edge/Brave/Firefox) 
1. Install Tampermonkey from the official store. https://www.tampermonkey.net/ 
2. Open Tampermonkey → “Create a new script”. 
3. Paste the userscript content (see “Script code”) and save. 
4. Open https://chatgpt.com (or https://chat.openai.com) and hard-reload (Ctrl+Shift+R). 
5. Verify in DevTools Console: [Otto MD] userscript loaded. 
6. Send a test: *italic* **bold** ***bold+italic*** ~~strikethrough~~ — it should render in your bubble. 


Usage 
• Alt+M toggles the renderer. The badge shows the current state. 
• To show literal symbols (* _ ~ `), escape them with backslashes: \*like this\*. 
• To disable completely — turn off the script in Tampermonkey or remove it. 


Notes & limits 
• Compact parser: tables and nested lists are not implemented. 
• DOM may change; if it breaks, update selectors targeting data-message-author-role="user". 
• Local-only. No network requests. No data exfiltration. 
...
Debugging 
• If it doesn’t run: ensure it’s enabled (green toggle) and domain matches @match. 
• Firefox: allow Tampermonkey in private windows; Injection Mode = Instant/Auto. 
• DevTools → Console should show: [Otto MD] userscript loaded. 
...
Install / remove 
• To update — overwrite the script contents. 
• To remove — Tampermonkey Dashboard → trash icon. 
• Export/backup and auto-updates can be configured in Tampermonkey Settings.
